...
class Upload {
  ...
  _createUpload() {
    ...
    xhr.onload = () => {
      ...
      // Location header can't get the whole URL, so we need to use the endpoint and add the last 2 parts of the Location URL
      // Which are the folder and the fileID
      let location = this.options.endpoint + xhr.getResponseHeader("Location").split('/').slice(-2).join('/');
      if (location == null) {
